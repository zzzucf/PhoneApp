...
public class VoiceMatch
{
	...
	public VoiceMatch(boolean typeofmatch, int nMFCCs, double fs, int nFilters,
			int FFTlength, String distFn, int radius)
	{
	...
	public double doDtwMatch(short[] audio1, short[] audio2)
	{
		...
		if (mfcc.preprocess(audio1, arrList1) > 0
				|| mfcc.preprocess(audio2, arrList2) > 0)
		{
		...
		} else
		{
			...
			for (int i = 0; i < arrList1.size(); i++)
			{
				...
				System.out.println(Integer.toString(i) + ' '
						+ Double.toString(sample1[i]));
			}
			for (int i = 0; i < arrList2.size(); i++)
			{
				sample2[i] = arrList2.get(i);
				System.out.println(Integer.toString(i) + ' '
						+ Double.toString(sample2[i]));
			}
			...
			final TimeWarpInfo info = com.dtw.FastDTW.getWarpInfoBetween(tsI,
					tsJ, m_FDtwRadius, m_distFn);
			return info.getDistance();
		}
	}
	...
	{
		short[] audio1 =
		{ 0, 0, 0, 0, 0, 0, 0, 1, 2, 3, 4, 5, 6, 9, 10, 3, 5, 6, 9, 2, 3, 0, 0,
				0, 0, 0 };
		short[] audio2 =
		{ 0, 0, 0, 0, 0, 0, 1, 2, 4, 4, 5, 6, 8, 9, 10, 10, 8, 8, 9, 2, 2, 2,
				2, 0, 0, 0, 0, 0, 0, 0 };
		VoiceMatch vm = new VoiceMatch(false, 13, 10.0, 24, 32,
		...
		System.out.println("Computing Time: " + String.valueOf(duration / 1e6)
				+ "ms");
	}
