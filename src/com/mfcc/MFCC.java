...
public class MFCC
{
	 ...
	 */
	public MFCC(int nnumberOfParameters, double dsamplingFrequency, int nnumberofFilters, int nFFTLength, boolean oisLifteringEnabled, int nlifteringCoefficient, boolean oisZeroThCepstralCoefficientCalculated)
	{
		...
		calculateMelBasedFilterBank(dsamplingFrequency, nnumberofFilters, nFFTLength);
		...
		if (m_oisLifteringEnabled)
		{
			...
			{
				new Error("Liftering is enabled and the number " + "of parameters = " + m_nnumberOfParameters + ", while " + "the liftering coefficient is " + m_nlifteringCoefficient + ". In this case some cepstrum coefficients would be made " + "equal to zero due to liftering, what does not make much " + "sense in a speech recognition system. You may want to " + "increase the liftering coefficient or decrease the number " + "of MFCC parameters.");
			}
		}
		else
		{
	 ...
	 */
	private void calculateMelBasedFilterBank(double dsamplingFrequency, int nnumberofFilters, int nfftLength)
	{
		...
		for (int i = 1; i <= nnumberofFilters; i++)
		{
			...
			for (int j = 1; j < dfftFrequenciesInMel.length - 1; j++)
			{
				...
				if ((dfftFrequenciesInMel[j] >= dfilterCenterFrequencies[i - 1]) & (dfftFrequenciesInMel[j] <= dfilterCenterFrequencies[i + 1]))
				{
		...
		for (int i = 0; i < nnumberofFilters; i++)
		{
			...
			{
				new Error("Error in MFCC filter bank. In filter " + i + " the first sample is equal to the last sample !" + " Try changing some parameters, for example, decreasing the number of filters.");
			}
		}
		...
		for (int i = 1; i <= nnumberofFilters; i++)
		{
			for (int j = m_nboundariesDFTBins[i - 1][0], k = 0; j <= m_nboundariesDFTBins[i - 1][1]; j++, k++)
			{
				...
				{
					m_dweights[i - 1][k] = (dfftFrequenciesInMel[j] - dfilterCenterFrequencies[i - 1]) / (dfilterCenterFrequencies[i] - dfilterCenterFrequencies[i - 1]);
				}
				else
				{
	...
	public double[] getParameters(double[] fspeechFrame)
	{
		...
		for (int i = 0; i < m_nnumberOfFilters; i++)
		{
			...
			if (m_dfilterOutput[i] > m_dminimumFilterOutput)
			{// floor power to avoid log(0)
	...
	public double[] getFilterBankOutputs(double[] fspeechFrame)
	{
		...
		for (int i = 0; i < m_nnumberOfFilters; i++)
		{
			...
			// ISIP (Mississipi univ.) implementation
			if (dfilterOutput[i] > m_dminimumFilterOutput)
			{// floor power to avoid log(0)
	...
	public int preprocess(short[] audioData, ArrayList<Double> sample)
	{
		...
		/*
		 * //System.out.println("a:"); //System.out.println(Arrays.toString(a));
		 * //System.out.println("id:");
		 * //System.out.println(Arrays.toString(id));
		 */
		// find sound segments isolated by silence
		int step = (int) Math.ceil(fs / 400), wSilence = (int) fs / 10, cnt = 0, nSegs; // assume
																						// the
																						// gap
																						// between
																						// two
																						...
																						// 0.1
																						// seconds
		int[] cursor = new int[20]; // store the start & end index of each
									// segment
		for (int i = 0; i < l;)
		{
		...
		for (int i = 0; i < nSegs; ++i)
		{
			...
			if (currentSegWidth > wUtterance)
			{ // assume a valid sample must be longer than 0.2s
		...
		if (maxSegPos == -1)
		{ // there is no valid speech sample
		...
		/*
		 * if (maxWidth >= fs/2) normed_sample = sample; else normed_sample =
		 * interpolate((int)fs, (int)(fs*fs/(2*maxWidth)), sample);
	...
	public double[] interpolate(double oldSampleRate, double newSampleRate, double[] samples)
	{
		...
		for (int i = 0; i < newLength; i++)
		{
			...
			double slope = samples[nearestRightPosition] - samples[nearestLeftPosition]; // delta
																							// x
																							// is
																							// 1
	...
	public double[][] doMFCC(double[] sample, double wintime, double hoptime)
	{
		...
		// int nFrames = 49; // samples are interpolated to fs/2 dimensions,
		// sliding window 50% overlap
		double[] frame = new double[winlength]; // one frame per 20ms containing
												// fs/50 sample points
	 ...
	 * IOException { int nnumberofFilters = 24; int nlifteringCoefficient = 22;
	 * boolean oisLifteringEnabled = true; boolean
	 * oisZeroThCepstralCoefficientCalculated = true; int
	 * nnumberOfMFCCParameters = 12; //without considering 0-th double
	 * dsamplingFrequency = 8820.0; // sampling rate 8kHz int nFFTLength = 256;
	 * if (oisZeroThCepstralCoefficientCalculated) //take in account the zero-th
	 * MFCC nnumberOfMFCCParameters = nnumberOfMFCCParameters + 1;
	 * 
	 * MFCC mfcc = new MFCC(nnumberOfMFCCParameters, dsamplingFrequency,
	 * nnumberofFilters, nFFTLength, oisLifteringEnabled, nlifteringCoefficient,
	 * oisZeroThCepstralCoefficientCalculated);
	 * 
	 * //System.out.println(mfcc.toString());
	 * 
	 * 
	 * int nClasses = 1; int[] nSamples = {10, 87, 74, 62}; int nWindows = 49;
	 * // samples are interpolated to fs/2 dimensions, sliding window 50%
	 * overlap double[] frame = new double[176]; // one frame per 20ms
	 * containing fs/50 sample points int numMFCCoef = 1; String path =
	 * "C:/Users/kai/workspace/MFCCExtraction/src/"; String[] sampleFileName =
	 * {"test_samples.txt", "c1_samples.txt", "c2_samples.txt",
	 * "c3_samples.txt"}; double[] mfccCoef; for (int i = 0; i < nClasses; ++ i)
	 * { double [][] samples = File.loadSample(path+sampleFileName[i],
	 * nSamples[i], (int)dsamplingFrequency/2); double [][] MFCCFeactureVec =
	 * new double[nSamples[i]][nWindows*numMFCCoef]; for (int j = 0 ; j <
	 * nSamples[i]; ++ j) { for (int k = 0; k < nWindows; ++ k) { frame =
	 * Arrays.copyOfRange(samples[j], (k + 1) * 88 - 88, (k + 1) * 88 + 87);
	 * mfccCoef = mfcc.getParameters(frame); for (int w = 0; w < numMFCCoef; w
	 * ++) { MFCCFeactureVec[j][k*numMFCCoef+w] = mfccCoef[w]; } } }
	 * File.saveFeature(path + "c" + Integer.toString(i) + "_features.svm",
	 * MFCCFeactureVec, i); //System.out.println( "MFCC parameters computed!");
	 * }
	 * 
	 * ///////////////////////////// /////////////////////////////
	 * ///////////////////////////// //simulate a frame of speech double[] x =
	 * new double[882]; // one frame per 20ms containing fs/50 sample points
	 * x[2] = 0.8; x[40] = 0.99;
	 * 
