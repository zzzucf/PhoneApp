...
public class TimeSeries
{
	...
	public TimeSeries(double[][] inputData)
	{
		this();
		labels.add("Time");
		for (int i = 0; i < inputData[0].length; ++i)
		{
			labels.add(new String("c" + i));
		}
		for (int i = 0; i < inputData.length; ++i)
		{
			timeReadings.add(new Double(timeReadings.size()));
			final TimeSeriesPoint readings = new TimeSeriesPoint(inputData[0]);
	...
	public TimeSeries(String inputFile, int[] colToInclude, boolean isFirstColTime, boolean isLabeled, char delimiter)
	{
		...
		try
		{
			...
			// time series file is not labeled
			{
